...
export class Home implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get('assets/campdata.csv', { responseType: 'text' }).subscribe(csvData => {
      // 使用 mapping function 將 CSV 轉成 CampSite[]
      this.campSites = this.mapCsvToCampSites(csvData);

      this.cities = Array.from(new Set(this.campSites.map(site => site.city)));
 ...
 */
  ...
  submitForm() {
    if (this.campForm.valid) {
      // 可將表單資料帶到結果頁
      this.router.navigate(['/result'], { state: { formData: this.campForm.value } });
    } else {
